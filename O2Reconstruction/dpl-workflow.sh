...
WORKFLOW="o2-ctf-reader-workflow $ARGS_ALL --delay $TFDELAY $NTIMEFRAMES_CMD --ctf-input $INPUT_FILE_LIST ${INPUT_FILE_COPY_CMD+--copy-cmd} ${INPUT_FILE_COPY_CMD} --onlyDet $WORKFLOW_DETECTORS $ARGS_EXTRA_PROCESS_o2_ctf_reader_workflow"
[[ -z "$DISABLE_ROOT_OUTPUT" ]] && WORKFLOW+=" | o2-tfidinfo-writer-workflow $ARGS_ALL"
WORKFLOW+=" | o2-mid-reco-workflow $ARGS_ALL $DISABLE_ROOT_OUTPUT $DISABLE_MC $ARGS_EXTRA_PROCESS_o2_mid_reco_workflow"
#WORKFLOW+=" | o2-mid-tracks-reader-workflow $ARGS_ALL $DISABLE_MC"
...
#WORKFLOW+=" | o2-mch-digits-writer-workflow $ARGS_ALL --enable-root-output --input-digits-data-description F-DIGITS --input-digitrofs-data-description F-DIGITROFS"
WORKFLOW+=" | o2-muon-tracks-matcher-workflow $ARGS_ALL $DISABLE_DIGIT_ROOT_INPUT $DISABLE_ROOT_OUTPUT $DISABLE_MC"
