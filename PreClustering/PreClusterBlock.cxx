...
int PreClusterBlock::reset(void* buffer, uint32_t size, bool write)
{
  ...
  if (size >= SSizeOfUShort) {
    ...
    if (mWriteMode) {
      cout << "The buffer is too small to store the data block." << endl;
      status = -ENOBUFS;
    } else {
      cout << "The buffer is too small to contain the data block." << endl;
      status = -EILSEQ;
    }
  }
...
int PreClusterBlock::startPreCluster(const DigitStruct& digit)
{
  ...
  if (mCurrentSize > mSize4PreCluster) {
    cout << "The buffer is too small to store a new precluster." << endl;
    mLastNDigits = nullptr;
    return -ENOBUFS;
  }
...
int PreClusterBlock::addDigit(const DigitStruct& digit)
{
  ...
  if (mCurrentSize > mSize4Digit) {
    cout << "The buffer is too small to store a new digit." << endl;
    return -ENOBUFS;
  }

  if (!mLastNDigits) {
    cout << "No precluster to attach the new digit to." << endl;
    return -EPERM;
  }
...
int PreClusterBlock::readBuffer()
{
  ...
  for (int i = 0; i < *mNPreClusters; ++i) {
    ...
    if (mCurrentSize < mSize) {
      ...
      cout << "Cannot read the expected number of preclusters." << endl;
      status = -EILSEQ;
      break;
    }
    ...
    if (nDigits && (*nDigits) > 0 && mCurrentSize + (*nDigits) * SSizeOfDigit <= mSize) {
      ...
      if (*nDigits == 0) {
        cout << "The precluster cannot contain 0 digit." << endl;
      } else {
        cout << "Cannot read the expected number of digits." << endl;
      }
  ...
  if (mCurrentSize != mSize && status >= 0) {
    cout << "The number of bytes read differs from the buffer size" << endl;
    status = -EILSEQ;
  }
...
std::ostream& operator<<(std::ostream& stream, const PreClusterStruct& cluster)
{
  ...
  for (int i = 0; i < cluster.nDigits; ++i) {
    stream << ", digit[" << i << "]= " << cluster.digits[i].uid;
  }
  stream << "}";
