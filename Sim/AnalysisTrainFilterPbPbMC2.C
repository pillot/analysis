...
TString     visible_name       = Form("FILTER%s$2", train_tag.Data()); //# FIXED #
...
TString     mergeDirName       = "";
// Make replicas on the storages below
TString     outputStorages      = "disk=3";
...
Int_t       iESDMCLabelAddition= 1;      // Add muon MC labels
...
Int_t       iMUONRefitVtx      = 1;      // Extrapolate muon tracks to another vtx before producing AODs
...
// TString     configPWG3d2h      = "$ALICE_ROOT/PWGHF/vertexingHF/ConfigVertexingHF.C";
...
void AnalysisTrainNew(const char *analysis_mode="grid", 
		      ...
		      const char *runListName="runList.txt",
                      const char *config_file="")
{
  ...
  if (iMUONRefit)   printf("=  MUON refit                                                    =\n");
  if (iMUONRefitVtx)printf("=  MUON refit vtx                                                 =\n");
  ...
  if (iMUONRefit || iMUONRefitVtx) {
    
    TString cdir = gSystem->WorkingDirectory();
    gSystem->ChangeDirectory(train_name);
    
    gSystem->Exec("cp -f $WORK/Macros/MuonPhysics/AddTaskMuonPhysics.C AddTaskMuonPhysics.C");
    gSystem->Exec("cp -f $WORK/Macros/MuonPhysics/AliAnalysisTaskMuonPhysics.cxx AliAnalysisTaskMuonPhysics.cxx");
    gSystem->Exec("cp -f $WORK/Macros/MuonPhysics/AliAnalysisTaskMuonPhysics.h AliAnalysisTaskMuonPhysics.h");
    
    if (iMUONRefit) {
      gSystem->Exec("cp -f $ALICE_ROOT/PWG/muondep/AliAnalysisTaskMuonRefit.cxx AliAnalysisTaskMuonRefit.cxx");
      gSystem->Exec("cp -f $ALICE_ROOT/PWG/muondep/AliAnalysisTaskMuonRefit.h AliAnalysisTaskMuonRefit.h");
    }
    
    if (iMUONRefitVtx) {
      gSystem->Exec("cp -f $WORK/Macros/Refit/AddTaskMuonRefitVtx.C AddTaskMuonRefitVtx.C");
      gSystem->Exec("cp -f $WORK/Macros/Refit/AliAnalysisTaskMuonRefitVtx.cxx AliAnalysisTaskMuonRefitVtx.cxx");
      gSystem->Exec("cp -f $WORK/Macros/Refit/AliAnalysisTaskMuonRefitVtx.h AliAnalysisTaskMuonRefitVtx.h");
    }
  ...
  if (iESDMCLabelAddition) {
    TString cdir = gSystem->WorkingDirectory();
    gSystem->ChangeDirectory(train_name);
    gSystem->Exec("cp -f $ALICE_ROOT/PWG/muondep/AliAnalysisTaskESDMCLabelAddition.cxx AliAnalysisTaskESDMCLabelAddition.cxx");
    gSystem->Exec("cp -f $ALICE_ROOT/PWG/muondep/AliAnalysisTaskESDMCLabelAddition.h AliAnalysisTaskESDMCLabelAddition.h");
    gSystem->ChangeDirectory(cdir);
  }
  ...
  //                                                                                                                                      
  if (usePLUGIN) {                                                                                                                        
    AliAnalysisGrid *alienHandler = CreateAlienHandler(plugin_mode, runListName);                                                                     
...
void AddAnalysisTasks(){                                                                                                                                          
  ...
  if (iMUONRefit || iMUONRefitVtx) {
    ...
    gROOT->LoadMacro(Form("%s/AddTaskMuonPhysics.C",train_name.Data()));
    AliAnalysisTaskMuonPhysics* physics1 = AddTaskMuonPhysics("_before");
    if (usePhysicsSelection) physics1->SelectCollisionCandidates(AliVEvent::kMUL7 | AliVEvent::kMUU7 |
								 AliVEvent::kMuonLikeLowPt8 | AliVEvent::kMuonUnlikeLowPt8);
    if (useCentrality) physics1->SelectCentrality(0., 90.);
    ...
    // second task (refit vtx)
    if (iMUONRefitVtx) {
      gROOT->LoadMacro(Form("%s/AddTaskMuonRefitVtx.C",train_name.Data()));
      AliAnalysisTaskMuonRefitVtx* refitVtx = AddTaskMuonRefitVtx(kFALSE, kTRUE);
    }
    
    // third task (physics after refit)
    AliAnalysisTaskMuonPhysics* physics2 = AddTaskMuonPhysics("_after");
    if (usePhysicsSelection) physics2->SelectCollisionCandidates(AliVEvent::kMUL7 | AliVEvent::kMUU7 |
								 AliVEvent::kMuonLikeLowPt8 | AliVEvent::kMuonUnlikeLowPt8);
    if (useCentrality) physics2->SelectCentrality(0., 90.);
  ...
  if (iESDfilter && !iAODanalysis) {
    ...
    if (iMUONcopyAOD) {
      ...
      AliAnalysisTaskESDfilter *taskesdfilter = AddTaskESDFilter(useKFILTER, kTRUE, kFALSE, kFALSE /*usePhysicsSelection*/,kFALSE,kTRUE,kFALSE,kFALSE,run_flag,1); // others  
    } else {
      ...
      AliAnalysisTaskESDfilter *taskesdfilter = AddTaskESDFilter(useKFILTER, kFALSE, kFALSE, kFALSE /*usePhysicsSelection*/,kFALSE,kTRUE,kFALSE,kFALSE,run_flag,1); // others
    }   
    /*
     Double_t v0Cuts[7];
...
Bool_t LoadAnalysisLibraries(const char *mode)
{
  ...
  if (iMUONRefit || iMUONRefitVtx) {
    ...
    if (!LoadLibrary("RAWDatabase", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("CDB", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("STEER", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONcore", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONmapping", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONcalib", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONgeometry", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONtrigger", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONraw", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONbase", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONrec", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("PWGmuon", mode, kTRUE)) return kFALSE;
    ...
    if (iMUONRefit) gROOT->LoadMacro("AliAnalysisTaskMuonRefit.cxx+");
    if (iMUONRefitVtx) gROOT->LoadMacro("AliAnalysisTaskMuonRefitVtx.cxx+");
    gSystem->ChangeDirectory(cdir);
  }
  
  if (iESDMCLabelAddition) {
//    if (!LoadLibrary("PWGmuondep", mode, kTRUE))) return kFALSE;
    if (!LoadLibrary("RAWDatasim", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONsim", mode, kTRUE)) return kFALSE;
    if (!LoadLibrary("MUONevaluation", mode, kTRUE)) return kFALSE;
    TString cdir = gSystem->WorkingDirectory();
    gSystem->ChangeDirectory(train_name);
    gROOT->LoadMacro("AliAnalysisTaskESDMCLabelAddition.cxx+");
    gSystem->ChangeDirectory(cdir);
  }
...
//______________________________________________________________________________
AliAnalysisAlien* CreateAlienHandler(const char *plugin_mode, const char *runListName)
{
  ...
  if (iMUONRefit || iMUONRefitVtx) {
    ana_sources += "AliAnalysisTaskMuonPhysics.cxx";
    anaLibs += "AliAnalysisTaskMuonPhysics.cxx AliAnalysisTaskMuonPhysics.h";
    if (iMUONRefit) {
      ana_sources += " AliAnalysisTaskMuonRefit.cxx";
      anaLibs += " AliAnalysisTaskMuonRefit.cxx AliAnalysisTaskMuonRefit.h";
    }
    if (iMUONRefitVtx) {
      ana_sources += " AliAnalysisTaskMuonRefitVtx.cxx";
      anaLibs += " AliAnalysisTaskMuonRefitVtx.cxx AliAnalysisTaskMuonRefitVtx.h";
    }
    ...
    plugin->AddIncludePath("-I$ALICE_ROOT/PWG/muon");
    plugin->AddIncludePath("-I.");
  }
  
  if (iESDMCLabelAddition) {
    ana_sources += " AliAnalysisTaskESDMCLabelAddition.cxx";
    anaLibs += " AliAnalysisTaskESDMCLabelAddition.cxx AliAnalysisTaskESDMCLabelAddition.h";
  }
...
void WriteConfig()
{
  ...
  out << "   iESDMCLabelAddition = " << iESDMCLabelAddition << ";" << endl;
  out << "   iESDfilter      = " << iESDfilter << ";" << endl;
  out << "   iMUONcopyAOD    = " << iMUONcopyAOD << ";" << endl;
  out << "   iMUONRefit    = " << iMUONRefit << ";" << endl;
  out << "   iMUONRefitVtx = " << iMUONRefitVtx << ";" << endl;
