...
//==================   TRAIN NAME   ============================================
TString     train_name         = "JPsiAOD"; // local folder name
TString     train_tag          = "_JPsi";        // Train special tag appended to 
...
TString     job_comment        = "AOD MUON J/Psi PbPb 2.76 realistic (LHC10h)";
...
TString     root_version       = "v5-30-06";  // *CHANGE ME IF MORE RECENT IN GRID*
TString     aliroot_version    = "v5-02-17-AN";  // *CHANGE ME IF MORE RECENT IN GRID*                                          
// Change production base directory here (test mode)
TString     alien_datadir      = "/alice/cern.ch/user/p/ppillot/Sim/LHC11h/JPsiPb276pm/MUL/GeoIdealVtx0/ESDs/results";
// Work directory in GRID (DON'T CHANGE)
TString     grid_workdir       = "Sim/LHC11h/JPsiPb276pm/MUL/GeoIdealVtx0/AODs";
// Data pattern - change as needed for test mode
TString     data_pattern       = "*AliESDs.root";
// To be able to read AliESDsSignal.root files produced by Javier's embedding
Bool_t      useCustomMCHandler = kFALSE;
// Set the run range
                      ...
                      const char *config_file="")
{
  ...
  if (useMC && !iAODanalysis) {
    ...
    if (useCustomMCHandler) {
      gROOT->LoadMacro("$WORK/Macros/Sim/AliMCMuonEventHandler.cxx++g");
      mcHandler = new AliMCMuonEventHandler();
    } else mcHandler = new AliMCEventHandler();
  ...
  if (mgr->InitAnalysis()) {                                                                                                              
    ...
    if (useCustomMCHandler) {
      gSystem->Exec("cp -f $WORK/Macros/Sim/AliMCMuonEventHandler.cxx AliMCMuonEventHandler.cxx");
      gSystem->Exec("cp -f $WORK/Macros/Sim/AliMCMuonEventHandler.h AliMCMuonEventHandler.h");
    }
...
{                                                                                                                                          
  ...
  if (iESDfilter && !iAODanalysis) {
    ...
    if (iMUONcopyAOD) {
      gROOT->LoadMacro("$WORK/Macros/Sim/AddTaskESDFilter.C");
      printf("Registering delta AOD file\n");
      mgr->RegisterExtraFile("AliAOD.Muons.root");
      //mgr->RegisterExtraFile("AliAOD.Dimuons.root");
      AliAnalysisTaskESDfilter *taskesdfilter = AddTaskESDFilter(useKFILTER, kTRUE, kFALSE, usePhysicsSelection, kFALSE, AliESDpid::kTOF_T0, kTRUE);
    } else {
  ...
  switch (imode) {
	...
	if (!local_xmldataset.Length()) {
	  ...
	  if (gSystem->AccessPathName("AliESDs.root")) 
	    ...
	    if (!saveTrain) chain->Add("AliESDs.root");
...
AliAnalysisAlien* CreateAlienHandler(const char *plugin_mode, const char *runListName)
{
  ...
  if (useCustomMCHandler) {
    ana_sources += " AliMCMuonEventHandler.cxx";
    anaLibs += " AliMCMuonEventHandler.cxx AliMCMuonEventHandler.h";
  }
  ...
  if (iMUONcopyAOD) plugin->SetOutputArchive("log_archive.zip:std*@disk=1 root_archive.zip:AliAOD*.root@disk=2");
  else plugin->SetOutputArchive("log_archive.zip:std*@disk=1 root_archive.zip:AliAOD.root@disk=2");
  ...
  plugin->SetValidationScript("JPsiAODvalidation.sh");
...
void WriteConfig()
{
  ...
  out << "   saveTrain       = " << saveTrain << ";" << endl << endl;
