...
enum PDC10Polar_t 
{
  kNO_pol, kTH_pol, kT_pol, kLH_pol, kL_pol, kPolMax 
};

const char * polarName[] = {
  "NO_pol", "TH_pol", "T_pol", "LH_pol", "L_pol" 
};
...
static PDC06Proc_t   proc     = kJPsi7TeV;
static PDC10Polar_t  polar    = kNO_pol;
...
void ProcessEnvironmentVars()
{
    ...
    // Polarization
    if (gSystem->Getenv("CONFIG_POLAR")) {
      for (Int_t iPol = 0; iPol < kPolMax; iPol++) {
        if (strcmp(gSystem->Getenv("CONFIG_POLAR"), polarName[iPol])==0) {
	  polar = (PDC10Polar_t) iPol;
	  cout<<"Polarization set to "<<polarName[iPol]<<" GeV"<<endl;
        }
      }
    }
