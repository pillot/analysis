...
TString     train_tag          = "_Xe-Xe_";  // Train special tag appended to visible name. ("data", "sim", "pp", "highmult", ...)
Int_t       iCollision         = 1;        // 0=pp, 1=Pb-Pb
Int_t       run_flag           = 1500;     // 1000 year 2010 LHCbcde cuts, 1100 year 2011 pp cuts, 1500 year 2015
TString     periodName         = "LHC17n"; // Set the period name, not only for testing, but also passed by a couple of tasks as args
Int_t       run_number         = 280234;        // For tasks arguments, CDB connect
Bool_t      localRunning       = kTRUE;   // Missing environment vars will cause a crash; change it to kTRUE if running locally w/o env vars
...
TString     aliphysics_version = "v5-09-18-01-1";     // *CHANGE ME IF MORE RECENT IN GRID*
...
TString     alien_datadir      = "/alice/data/2017/LHC17n";
// Work directory in GRID (DON'T CHANGE)
TString     grid_workdir       = "Data/LHC17n/muon_calo_pass1/RefitAODs";
// Data pattern - change as needed for test mode
TString     data_pattern       = "/muon_calo_pass1/*ESDs.root";
Int_t       run_numbers[10]    = {280234,280235}; // Set the run range, for testing
...
// Optionally define the files to be archived.
   ...
   plugin->SetOutputFiles("AliAOD.Muons.root");
   plugin->SetOutputArchive("root_archive.zip:AliAOD.Muons.root@disk=2");
