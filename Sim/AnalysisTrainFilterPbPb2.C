...
TString     root_version       = "v5-34-02";  // *CHANGE ME IF MORE RECENT IN GRID*
TString     aliroot_version    = "v5-03-72-AN";  // *CHANGE ME IF MORE RECENT IN GRID*                                          
// Change production base directory here (test mode)
TString     alien_datadir      = "/alice/data/2011/LHC11d";
Int_t       run_flag           = 1100;   // year (2011 = 1100)

// Work directory in GRID (DON'T CHANGE)
TString     grid_workdir       = "Data/LHC11d/pass2_muon/AODs";
// Data pattern - change as needed for test mode
TString     data_pattern       = "*ESDs/pass2_muon/*ESDs.root";
// Set the run range
...
Bool_t LoadAnalysisLibraries(const char *mode)
{
  ...
  if (iMUONRefit) {
    ...
    gROOT->LoadMacro("AliAnalysisTaskMuonRefit.cxx+");
    gSystem->ChangeDirectory(cdir);
  }
...
AliAnalysisAlien* CreateAlienHandler(const char *plugin_mode, const char *runListName)
{
  ...
  if (iMUONRefit) {
    ana_sources += "AliAnalysisTaskMuonPhysics.cxx AliAnalysisTaskMuonRefit.cxx";
    anaLibs += "AliAnalysisTaskMuonPhysics.cxx AliAnalysisTaskMuonPhysics.h AliAnalysisTaskMuonRefit.cxx AliAnalysisTaskMuonRefit.h";
