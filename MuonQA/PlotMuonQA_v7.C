...
// ALIROOT includes
#include "../PWG3/muon/AliCounterCollection.h"

#endif


// .x PlotMuonQA_v7.C("alien:///alice/cern.ch/user/c/cynthia/muon/QA/pp/LHC10e/pass2_test/results",0,kFALSE)
// .x PlotMuonQA_v7.C("/Users/cynthia/Documents/alice/data/MuonQA/results",0,kFALSE)
// .x PlotMuonQA_v7.C("/Users/cynthia/Documents/alice/data/MuonQA/results","/Users/cynthia/Documents/alice/data/MuonQA/LHC10e/pass2/runlist_period3_test3_3runs.txt",kFALSE)
//--------------------------------------------------------------------------
void PlotMuonQA_v7(const char* baseDir, const char* runList = 0x0, Bool_t selectPhysics = kFALSE)
{
  /// Macro for QA monitoring.
  ...
  hCMUS1ACNoPS->Sumw2();
  Int_t NumOfCMUS1ACNoPS = hCMUS1ACNoPS->Integral();
//  TH1* hCMUS1ENoPS = eventCounters->Draw("run",Form("trigger:CMUS1E/%s", selectRuns.Data()));
//  hCMUS1ENoPS->Sumw2();
//  Int_t NumOfCMUS1ENoPS = hCMUS1ENoPS->Integral();
...
//  ratioBckCMUS1BNoPS->Add(hCMUS1ENoPS,2);
  ...
  TH1D *hAsymMatched= static_cast<TH1*>(hDiffMatched->Clone("hAsymMatched"));
  ...
  for ( Int_t irun=0; irun<runs.GetEntriesFast(); irun++ ) {
	...
	while(kTRUE){
	  iEntry++;
	  dirFilename = gSystem->GetDirEntry(dir);
	...
	continue;//return;
      ...
      TH1* hNClustersPerCh = general2 ? static_cast<TH1*>(general2->FindObject("hNClustersPerCh")) : 0x0;
